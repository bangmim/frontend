...
import {Tab, Button, Divider, Image, Transition,Icon,Segment,List, TableBody,Table, Modal } from 'semantic-ui-react'
...
import { PieChart, Pie, Sector, Radar, RadarChart, PolarGrid, PolarAngleAxis, ResponsiveContainer, BarChart, Bar, Cell, XAxis, YAxis, CartesianGrid, Tooltip, Legend } from 'recharts';
...
function App() {
  ...
  {
    menuItem: <Button animated='vertical' size='large' color='black'   >
   ...
   <Rechart data={data} data1={selectedcity} data2={selectedgu} data3={selectSago} />
   </Tab.Pane>
   </div>,
  },
  {
    menuItem: <Button animated='vertical' size='large'  color='blue'>
  ...
  {
    menuItem: <Button animated='vertical' size='large'  color='yellow'>
    ...
    <Table.Header>
      <Table.Row textAlign='center'>
        ...
        <Table.HeaderCell >시군구</Table.HeaderCell>
        <Table.HeaderCell >사고유형</Table.HeaderCell>
        <Table.HeaderCell >발생년월일시</Table.HeaderCell>
        <Table.HeaderCell >요일</Table.HeaderCell>
        <Table.HeaderCell >주/야간</Table.HeaderCell>
        <Table.HeaderCell >부상자수</Table.HeaderCell>
        <Table.HeaderCell >중상자수</Table.HeaderCell>
        <Table.HeaderCell >사망자수</Table.HeaderCell>
      </Table.Row>
    </Table.Header>
    <Table.Body>
      <Table.Row textAlign='center'>
    ...
    <Table.Cell> {횡단중.map((item, index) => (
    <p key={index}> {item.요일} </p>
    ))}</Table.Cell>
    <Table.Cell> {횡단중.map((item, index) => (
    <p key={index}> {item.주야} </p>
    ))}</Table.Cell>
    <Table.Cell> {횡단중.map((item, index) => (
    ...
    <p key={index}> {item.발생지시도} </p>
        ...
        <Table.Cell> {측면충돌.map((item, index) => (
    <p key={index}> {item.발생지시군구} </p>
    ))}</Table.Cell>
    <Table.Cell> {측면충돌.map((item, index) => (
    <p key={index}> {item.사고유형} </p>
    ...
    <p key={index}> {item.발생지시도} </p>
        ...
        <Table.Cell> {공작물충돌.map((item, index) => (
    <p key={index}> {item.발생지시군구} </p>
    ))}</Table.Cell>
    <Table.Cell> {공작물충돌.map((item, index) => (
    <p key={index}> {item.사고유형} </p>
...
const selectGu = data.data.filter(item => selectedcity === item["발생지시도"])
...
const selectSago = data.data.filter(item => selectedgu === item["발생지시군구"])
  .map(item => item);
      ...
      <Divider hidden />
        ...
        <div>
          <Box sx={{ minWidth: 120 }} >
            ...
            <FormControl variant="standard" sx={{ m: 1, minWidth: 80 }}>
              ...
              <Select  onChange={handleSelect2} value={selectedgu || ''}>
                ...
                <button class="ui animated button" onClick={toggleVisibility}  style={{borderRadius:'50%', height: '30px', width: '30px', marginTop: '20px', backgroundColor: "black"}} >
            <div class="visible content"></div>
            <div class="hidden content">
              <i aria-hidden="true" class="search icon" style={{paddingLeft: '4px', color: 'white'}}></i>
            </div>
          </button>
          </Box>
          <Divider hidden />
          ...
          <Transition visible={visible} animaiton='drop' duration={800} >
            <Button class="ui icon button fade visible transition" style={{backgroundColor: "white", color:"black"}} onClick={handlerest}  size='big'>
            <Icon name='close' />
            </Button>
          </Transition>
          </div>
  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      ...
      >
        ...
        <Tooltip />
        <Legend />
        <Bar dataKey="중상자수" stackId="a" fill="#47B5FF" />
        <Bar dataKey="부상자수" stackId="a" fill="#1363DF" />
        <Bar dataKey="사망자수" stackId="a" fill="#06283D" />
...
function KakaoMap({ data, data1, data2, data3 }) {
  ...
  const 기타 = 선택1.filter(item => {
    if ((item["발생지시도"] === "인천") && (!(item["사고유형"] === "공작물충돌") && !(item["사고유형"] === "추돌") && !(item["사고유형"] === "측면충돌") && !(item["사고유형"] === "횡단중"))) {
      return item;
    }
  })
  ...
  return (
    <div style={{ width:"100%",height:"80%", padding:"0 4rem 4rem 4rem"}}> 
      ...
      {전체사고경도위도.map((position, index) => {
        ...
        if (!(position.title === "공작물충돌") && !(position.title === "추돌") && !(position.title === "측면충돌") && !(position.title === "횡단중")) {
          return (
            <MapMarker
              key={`${position.title}-${position.latlng}`}
              position={position.latlng} // 마커를 표시할 위치
